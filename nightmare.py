...
def normalize_url(url: str) -> str:
    return spider_url_policy.normalize_url(url)
...
def normalize_url_for_sitemap(url: str) -> str:
    return spider_url_policy.normalize_url_for_sitemap(url)


def normalize_path_segment_for_condensed_tree(segment: str) -> str:
    return spider_url_policy.normalize_path_segment_for_condensed_tree(segment)


def is_allowed_domain(url: str, root_domain: str) -> bool:
    return spider_url_policy.is_allowed_domain(url, root_domain)


def should_crawl_url(url: str) -> bool:
    ensure_extension_config_loaded()
    return spider_url_policy.should_crawl_url(
        url,
        static_asset_extensions=STATIC_ASSET_EXTENSIONS,
        high_value_extensions=HIGH_VALUE_EXTENSIONS,
        high_value_path_suffixes=HIGH_VALUE_PATH_SUFFIXES,
    )


def should_include_url_in_ai_payload(url: str) -> bool:
    ensure_extension_config_loaded()
    return spider_url_policy.should_include_url_in_ai_payload(url, AI_EXCLUDED_URL_EXTENSIONS)


def filter_urls_for_ai(urls: list[str]) -> list[str]:
    ensure_extension_config_loaded()
    return spider_url_policy.filter_urls_for_ai(urls, AI_EXCLUDED_URL_EXTENSIONS)


def should_include_url_in_source_of_truth(url: str) -> bool:
    ensure_extension_config_loaded()
    return spider_url_policy.should_include_url_in_source_of_truth(url, STATIC_ASSET_EXTENSIONS)


def filter_urls_for_source_of_truth(urls: list[str]) -> list[str]:
    ensure_extension_config_loaded()
    return spider_url_policy.filter_urls_for_source_of_truth(urls, STATIC_ASSET_EXTENSIONS)
...
def _looks_like_hashed_bundle_filename(filename: str) -> bool:
    return spider_url_policy._looks_like_hashed_bundle_filename(filename)


def summarize_url_for_ai(url: str) -> str:
    return spider_url_policy.summarize_url_for_ai(url)


def condense_urls_for_ai(urls: list[str]) -> list[str]:
    ensure_extension_config_loaded()
    return spider_url_policy.condense_urls_for_ai(urls, AI_EXCLUDED_URL_EXTENSIONS)
...
def url_path_matches_high_value_suffix(url: str) -> str | None:
    ...
    ensure_extension_config_loaded()
    return spider_url_policy.url_path_matches_high_value_suffix(url, HIGH_VALUE_PATH_SUFFIXES)
...
def main() -> None:
    install_error_reporting(program_name="nightmare", component_name="crawler", source_type="worker_tool")
    restore_default_interrupt_handlers()
...
if __name__ == "__main__":
    try:
        ...
        logger = logging.getLogger("nightmare")
        report_error(
            "Fatal error during nightmare execution",
            program_name="nightmare",
