...
echo "Local URL: http://localhost (recommended for host-side coordinator / VS Code; avoids TLS to :443)"
echo "HTTPS:     https://localhost (same API; use if you prefer TLS to published :443)"
...
echo "  - VS Code Debug Coordinator uses COORDINATOR_BASE_URL=http://localhost (see .vscode/launch.json)"
