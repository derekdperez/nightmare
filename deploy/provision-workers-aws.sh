...
#cloud-config
runcmd:
  - [bash, -lc, "set -euo pipefail; if command -v yum >/dev/null 2>&1; then yum makecache -y || true; yum install -y ca-certificates curl-minimal git docker openssl; yum install -y docker-compose-plugin || yum install -y docker-compose || true; elif command -v dnf >/dev/null 2>&1; then dnf makecache -y || true; dnf install -y ca-certificates curl-minimal git docker openssl; dnf install -y docker-compose-plugin || dnf install -y docker-compose || true; elif command -v apt-get >/dev/null 2>&1; then apt-get update; apt-get install -y ca-certificates curl git docker.io docker-compose-plugin openssl; else echo 'no supported package manager' >&2; exit 1; fi"]
